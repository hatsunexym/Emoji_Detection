...
if __name__ == "__main__":
    ...
    while (1) : 
        ...
        cursor.execute(sql)
        db.commit()
        sql2 ="insert into  person_dect(id,exis_person,num,videoname,videourl)value("+id+","+exis_person+","+str(num)+","+"'"+"video2"+"'"+","+"'"+"rtsp://admin:Chen1qaz@192.168.1.108/cam/realmonitor?channel=1&subtype=0"+"'"+")"
        cursor.execute(sql2)
        db.commit()
        ...
        print (ti[-2:])
        name = '/home/chen/apache-tomcat-7.0.81/webapps/printscreen/2/'+ti[-2:]+'.jpg'
        b= b+1
        cv2.imwrite(name,im)
